...
contract BtswapRouter is IBtswapRouter02, Ownable {
    ...
    constructor(address _factory, address _WETH, address _minter) public {
        factory = _factory;
        WETH = _WETH;
        minter = _minter;
    }
    ...
    function addLiquidity(
        ...
        IBtswapMinter(minter).liquidity(msg.sender, pair);
    }
    ...
    function onTransfer(address sender, address recipient) public onlyPair returns (bool) {
        IBtswapMinter(minter).liquidity(sender, msg.sender);
        IBtswapMinter(minter).liquidity(recipient, msg.sender);
